...
`daily_to_blog_voice.py` 的 `--audio` 仅作**一键省事**；技能默认要求**先完成第 2 步再执行第 3 步**，避免未审稿就定稿音频。

脚本：`scripts/daily_to_blog_voice.py`（解析 `今日资讯` 条目，去掉原文/来源行与 URL，再拼开场、分条、`###` 分段、收尾与附录链接）。
